...
\newline
\newline
Finally, we experimented with tweaks in the architecture and optimization methods to see if we could get any final small improvements in our pseudo-labels model. We experimented with the momentum in the SGD optimizer, batch normalization, increasing the number of fully connected layers, and with using the Adam optimizer instead of SGD optimizer. Our experiments showed that using batch normalization on the 2nd convolutional layer and using the Adam optimizer yielded small improvements in performance. 
